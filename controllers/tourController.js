...
exports.createTour = async (req, res) => {
  try {
    ...
    res.status(400).json({
      status: 'fail',
      message: err, // Should not be used for production
    });
  }
};
...
exports.updateTour = async (req, res) => {
  try {
    ...
    res.status(400).json({
      status: 'fail',
      message: 'Invalid data sent!', // Should not be used for production
    });
  }
};
...
exports.deleteTour = async (req, res) => {
  try {
    await Tour.findByIdAndDelete(req.params.id); // We dont save this to a alue
    res.status(204).json({
